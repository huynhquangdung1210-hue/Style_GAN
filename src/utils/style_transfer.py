...
class NeuralStyleTransfer(StyleTransferModel):
    ...
    def _style_transfer_sync(
        ...
        # Get content image dimensions to preserve aspect ratio
        content_width, content_height = content_image.size
        
        # Adjust target size based on quality while preserving aspect ratio
        size_map = {"low": 256, "medium": 512, "high": 1024}
        max_size = size_map.get(quality, 512)
        
        # Calculate target dimensions preserving aspect ratio
        aspect_ratio = content_width / content_height
        if content_width > content_height:
            target_width = min(content_width, max_size)
            target_height = int(target_width / aspect_ratio)
        else:
            target_height = min(content_height, max_size)
            target_width = int(target_height * aspect_ratio)
        
        # Ensure we don't exceed max size in either dimension
        ...
        print(f"Content image: {content_image.size} -> Target size: {target_size}")
    ...
    def _preprocess_image(self, image: Image.Image, target_size: tuple) -> torch.Tensor:
        """Preprocess PIL image to tensor."""
        # Convert to RGB and resize to target dimensions
